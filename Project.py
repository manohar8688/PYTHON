...
skill_weights = {"python": 30, "java": 28, "sql": 25, "html": 10, "css": 8,"datastructures": 30, "communication": 10, "cloud": 25,"linux": 20, "testing": 12, "aiml": 22, "systemdesign": 26, "c": 20}
skill_aliases = {
    "python": ["py", "python3"],
    ...
    "c": ["c language"]}
Total_skill_weightage=0
for skill_marks in skill_weights.values():
    Total_skill_weightage+=skill_marks
...
if st.sidebar.button("🔄 Upload New Resume"):
    for k in ["name", "age", "resume_text", "uploaded_file", "option"]:
        if k in st.session_state:
            del st.session_state[k]
    st.rerun()
st.title("📄Resume Analysis & Placement Recommendation System")
name = st.text_input("👤 Enter your name", key="name", value=st.session_state.get("name",""))
age = st.number_input("👤Enter your age", min_value=10, max_value=60, key="age", value=st.session_state.get("age",10))
if age < 18 or age > 26:
...
option = st.radio("Choose input type:", opts, key="option", index=opts.index(st.session_state.get("option",opts[0])))
if option == "📝Enter Resume Text":
    resume_text = st.text_area("Paste your resume text here", height=200, key="resume_text", value=st.session_state.get("resume_text",""))
